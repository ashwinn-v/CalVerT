...
| `TELEMETRY_AGENT_ROOT` | Root of this checkout; used by the agent loop to resolve `grpo/config/reward_config.yaml` |
| `CLUSTER_NAME` | Optional label written into result JSONLs for provenance |

## Models


- **Generator:** Qwen3-32B, Qwen3-8B (paper); Mistral-Small-24B-Instruct; any
  vLLM-compatible chat model with HF chat templates.
- **Grounding verifier:** `bespoke-mlfoundations/MiniCheck-Bespoke-MiniCheck-7B`.
- **NLI for DINCO:** `MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli`.
